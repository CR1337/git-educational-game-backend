from git_orchestrator.git_orchestrator_interface import GitOrchestratorInterface, ForbiddenPathException
...
class GameController:
    ...
    def new_game(cls, player: models.Player) -> models.Game:
        ...
        game_levels_directory = Filesystem.get_game_levels_path(game_id, must_exist=False)
...
class LevelController:
    ...
    def get_levels(cls, game_id: models.IdType) -> List[models.LevelNode]:
        ...
        for level_id in level_ids:
            ...
            game_level_metadata_filename = Filesystem.get_game_level_meta_path(game_id, level_id)
    ...
    @classmethod
    def reset_level(cls, game_id: models.IdType, level_id: models.IdType) -> models.Level:
        ...
        game_level_directory = Filesystem.get_game_level_path(game_id, level_id, must_exist=False)
        if game_level_directory.exists():
                shutil.rmtree(game_level_directory)
        game_level_directory.mkdir()

        level_meta_filename = Filesystem.get_game_level_meta_path(game_id, level_id, must_exist=False)
        ...
        game_level_repo_directory = Filesystem.get_game_level_repo_path(game_id, level_id, must_exist=False)
        ...
        for args in init_git_command_args:
            GitController.run_git_command_directly(
                game_id, level_id, args
            )
        ...
        for command in init_commands:
            ...
            match cmd:
                ...
                case "write":
                    ...
                    destination_filename = Filesystem.get_game_level_repo_file(game_id, level_id, destination, must_exist=False)
                ...
                case "remove":
                    assert len(args) == 1
                    basename = args[0]
                    filename = Filesystem.get_game_level_repo_file(game_id, level_id, basename)
                    os.remove(filename)

                case "create":
                    assert len(args) == 1
                    basename = args[0]
                    filename = Filesystem.get_game_level_repo_file(game_id, level_id, basename, must_exist=False)
...
class FileController:
    ...
    @classmethod
    def get_file(cls, game_id: models.IdType, level_id: models.IdType, filename: str) -> models.File:
    ...
    @classmethod
    def put_file(cls, game_id: models.IdType, level_id: models.IdType, filename: str, file: models.File) -> None:
        file_path = Filesystem.get_game_level_repo_file(game_id, level_id, filename)
...
class GitController:
    ...
    def _get_git_interface(
        ...
        interface = GitOrchestratorInterface(
            git_session_id,
            base_directory
        )
    ...
    @classmethod
    def run_git_command(cls, game_id: models.IdType, level_id: models.IdType, git_command: models.GitCommand) -> Union[models.GitResult, models.EditorRequest, None]:
    ...
    @classmethod
    def run_git_command_directly(cls, game_id: models.IdType, level_id: models.IdType, argv: List[str]) -> bool:
    ...
    @classmethod
    def handle_editor_response(cls, game_id: models.IdType, level_id: models.IdType, editor_response: models.EditorResponse) -> Union[models.GitResult, None]:
