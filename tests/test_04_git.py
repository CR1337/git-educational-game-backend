...
def test_post_editor_response(setup_and_teardown, game_id, level_id, filename):
    ...
    _, _ = make_request("put", f"/games/{game_id}/levels/{level_id}/files/{filename}", payload)
    
    payload = {
        "type_": "GitCommand",
        "argv": ["add", "."]
    }
    _, _ = make_request("post", f"/games/{game_id}/levels/{level_id}/git-command", payload)
    ...
    assert "Please enter the commit message for your changes." in data["file"]["content"]
    ...
    status, data = make_request("post", f"/games/{game_id}/levels/{level_id}/editor-response", payload)
